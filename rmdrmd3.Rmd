...
Findings:
They find that individual metrics are repeatable in both disturbed and undisturbed networks, but that group-level metrics are altered by disturbance. 
...
```{r}
#make sure things are factors, and levelled as in the paper
netPos$Focal.Letter.ID=as.factor(netPos$Focal.Letter.ID)
netPos$Sex=as.factor(netPos$Sex)
netPos$Tub=as.factor(netPos$Tub)

netPos <- within(netPos, Treatment <- relevel(Treatment, ref = "C"))
netPos <- within(netPos, Period <- relevel(Period, ref = "1"))
netPos <- within(netPos, Sex <- relevel(Sex, ref = "F"))
str(netPos)
```
...
The following function just performs intra-class correlation analysis. I have to run 6 analyses, so figured a function would be neater.
```{r, message = FALSE}
...
global_perms=1000 #all perm tests are with 10,000 perms, but I ran it with 1000 because 10000 took ages!
...
Results dataframe:
```{r}
df2=data.frame(metrics=c("strength", "betweenness", "clustering coef"))
...
The following function turns the SRI (association) scores in the dataframe into an association matrix. I needed to do this for plotting the networks. I didn't generalize the function as I am only using it for tehse dpecific datasets,
```{r}
...
```
```{r}
hist(perms_C6s, breaks=50, xlim=c(-0.2, 0.2))
...
for (i in 1:global_perms){
  temp=netPos[complete.cases(netPos), ]
  temp$betweenness=sample(temp$betweenness, length(temp$betweenness), replace = FALSE)
  summ=summary(lmer(betweenness~Treatment*Period+Sex+Elytra+Number.of.observations + (1|Focal.Letter.ID) + (1|Tub), data = temp))
...
for (i in 1:global_perms){
  temp=netPos[complete.cases(netPos), ]
  temp$Clustering.coefficient=sample(temp$Clustering.coefficient, length(temp$Clustering.coefficient), replace = FALSE)
  summ=summary(lmer(Clustering.coefficient~Treatment*Period+Sex+Elytra+Number.of.observations + (1|Focal.Letter.ID) + (1|Tub), data = temp))
...
Hmm... my results are way off of theirs! What gives?? Disagreement on whats significant, as well as the signs of some of the coefficient. I think the most likely explanation is that I permuted in a different way that they did. They did not specify what was permuted; the response or predictor vars. I went for the response var just because it would be easier, but it's possible they premuted each separate predictor var, which would alter the relationships among the precictor vars. This could produce different results. They did not go into detail about this in their paper... Oh well... But, the sign of the coefficients comes from the specified model, and isnt affected by permutations, so I don't really understand how these could be different unless I screwed up. There were 'singular fit' warnings for practically every model, so maybe this has something to do with it? But the main result; that treatmentXperiod is similarly significant and has the same sign, and that was the main research question, so maybe this was broadly successful?

Reflection:
Overall, the dataset was pretty easy to work with. But the details surrounding how the permutation  tests were conducted was sparse, and there are issues with how they analysed thde data which I have addressed elsewhere. 
 got broadly similar results, but the group-level metrics permutation threw up some weirdness!